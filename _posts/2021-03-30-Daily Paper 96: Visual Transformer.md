...
提到transformer，就不得不比较一下在CV领域得到广泛应用的卷积。卷积和自注意力最大的应用区别是局部和全局的关系，这也是由于卷积所具有的inductive bias和equivalent invariant所决定的。由于卷积具有天生的2D维度上的归纳偏置，那么卷积会通过大小一定的kernel天生的将附近的像素点考虑在内，这就使得卷积更注重局部的信息。而transformer刚好相反，由于每一个token之间的距离都被缩减至1，因此tranformer以及其本质的自注意力机制更倾向于从全局的角度捕获信息。  
...
这个没啥好讲的，就是把16个token扔到transformer计算相似度。  
